# AI Video Commentary Slicer

一个面向影视解说/短视频创作者的本地 AI 成片 demo。它以“配音文案优先”为核心流程：先从原视频提取语音字幕，再用大模型结合人工上下文包生成影视解说文案，经过语义审查和口播润色后生成 TTS 配音，最后按配音文案对应的原视频字幕时间戳剪辑画面、移除原声、混入可选 BGM，并导出带新配音和字幕的目标时长成片。
